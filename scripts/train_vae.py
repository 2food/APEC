...
def log_info(epoch):
    ...
    losses_str = " ".join(["{}: {:.4f}".format(x, y)
                           for x, y in zip(loss_names, train_losses)])
    logger.info(
        "====> Epoch: {} Time: {:.2f} {} lr: {:.5f} exp: {}".format(
            i, dt, losses_str, lr, exp_name)
    )
    if mode == 'train':
...
if __name__ == "__main__":
    ...
    if mode == "train":
        ...
        for i in range(args.iter, cfg.num_epoch):
            train_losses, dt = run_epoch(
                cfg, dataset, model, loss_func, optimizer, device, dtype=dtype, scheduler=scheduler, mode=mode)
            log_info(i)
