...
def flip_smpl(pose):
    ...
    curr_spose_euler = curr_spose.as_euler(
        'ZXY', degrees=False).reshape(pose.shape[0], 24, 3)
    curr_spose_euler = left_to_rigth_euler(curr_spose_euler)
    curr_spose_aa = sRot.from_euler("ZXY", curr_spose_euler.reshape(
        -1, 3), degrees=False).as_rotvec().reshape(pose.shape[0], 24, 3)
...
if __name__ == "__main__":
    ...
    for (k, v) in pbar:
        ...
        skips = np.unique([int(amass_fr / target_fr)
                           for target_fr in target_frs]).astype(int)

        seqs, start_points, trans = [], [], []
        for skip in skips:
            seqs_org, trans_org, start_points_org = sample_seq_length(
                amass_pose[::skip], amass_trans[::skip], seq_length)
            seqs_flip, trans_flip, start_points_flip = sample_seq_length(
                amass_pose_flip[::skip], amass_trans[::skip], seq_length)
