...
if __name__ == "__main__":
    ...
    meva_dir = 'results/meva/train_meva_all_vae30/model_best.pth.tar'
    # meva_dir = 'results/meva/16-11-2020_14-49-27_meva/model_best.pth.tar'
    checkpoint = torch.load(meva_dir, map_location=device)
    ...
    dataset_data = joblib.load(
        os.path.expanduser("~/ucph-erda-home/vibe_db/3dpw_test_db.pt"))
    ...
    for u_n in unique_names:
        ...
        if "valid" in dataset_data:
            ...
            valids = np.ones(
                dataset_data['features'][indexes].shape[0]).astype(bool)
    ...
    with torch.no_grad():
        ...
        for seq_name in pbar:
            ...
            if num_frames < t_total:
                ...
                chunk_idxes, chunck_selects = get_chunk_with_overlap(
                    num_frames, window_size=t_total, overlap=overlap)

            meva_theta, meva_j3d = [], []
            for curr_idx in range(len(chunk_idxes)):
                chunk_idx = chunk_idxes[curr_idx]
                cl = chunck_selects[curr_idx]
                meva_preds = meva_model(
                    curr_feat[None, chunk_idx, :], J_regressor=J_regressor)

                meva_theta.append(
                    meva_preds[-1]['theta'][0, cl[0]:cl[1], 3:75].cpu().numpy())
                meva_j3d.append(meva_preds[-1]['kp_3d']
                                [0, cl[0]:cl[1]].cpu().numpy())
            ...
            mpjpe, mpjpe_pa, error_vel, error_acc, errors_pck, mat_procs = compute_errors(
                meva_j3d * 1000, gt_j3d * 1000)
